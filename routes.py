...
def recommend_crop(current_user):
    try:
        ...
        return jsonify({"success": False, "message": "Parameters sahi nahi hain"}), 400
    ...
    try:
        ...
        top5      = sorted(zip(classes, probs), key=lambda x: -x[1])[:5]
    except Exception as e:
        return jsonify({"success": False, "message": str(e)}), 500
...
def rotation_plan(current_user):
    ...
    try:
        crop_enc = rotation_encoders['le_crop'].transform([current_crop])[0]
        soil_enc = rotation_encoders['le_soil'].transform([soil_type])[0]
        goal_enc = rotation_encoders['le_goal'].transform([goal])[0]
